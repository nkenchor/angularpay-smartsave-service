...
import io.angularpay.smartsave.exceptions.ValidationException;
import io.angularpay.smartsave.models.AccessControl;
...
import static io.angularpay.smartsave.helpers.Helper.*;
...
public abstract class AbstractCommand<T extends AccessControl, R> {
    ...
    public R execute(T request) {
        try {
            ...
            if (this instanceof UpdatesPublisherCommand && response instanceof SmartSaveRequestSupplier) {
                log.info("publishing {} update to REDIS => message payload:  {}", this.name, responseText);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UpdatesPublisherCommand)this).publishUpdates((SmartSaveRequestSupplier) response);
                });
            }
            if (this instanceof UserNotificationsPublisherCommand && response instanceof SmartSaveRequestSupplier) {
                log.info("publishing {} User Notification to REDIS", this.name);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UserNotificationsPublisherCommand)this).publishUserNotification((SmartSaveRequestSupplier)response);
                });
            }
